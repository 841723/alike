...
import lib.symbolTable.exceptions.ReservedWordException;
...
public class SymbolTable {
    ...
    public int getNumVarsToSave(int level) {
        int total = 0;
        HashMap<String, Symbol> stl = st.get(st.size() - 1);
        for (Map.Entry<String, Symbol> entry : stl.entrySet()) {
            Symbol s = entry.getValue();
            if (s.type != Types.PROCEDURE && s.type != Types.FUNCTION) {
                total++;
            }
        }
        return total;
    ...
    public void insertSymbol(Symbol s) throws AlreadyDefinedSymbolException {
        ...
        if (currentBlock.containsKey(s.name)) { // ya está
            ...
            s.nivel = level;
            s.dir = currentBlock.size() + 3;
            if (s.type == Types.ARRAY && s.parClass != Symbol.ParameterClass.REF) {
                SymbolArray sa = (SymbolArray) s;
                currentBlock.put(s.name, sa);
                for (int i = sa.minInd + 1; i <= sa.maxInd; i++) {
                    currentBlock.put(s.name + "[" + i + "]", sa);
                }
            } else {
    ...
    public Symbol getSymbol(String name) throws SymbolNotFoundException {
        ...
        if (result == null) {
            System.err.println("ERR-03");
            throw new SymbolNotFoundException(name);
        }
        return result;
    }

    public boolean existsSymbol(String name) {
        return findSymbol(name) != null;
    }

    public int getPositionInBlock(String name) {
        return (int) getSymbol(name).dir;
    }

    public int getTmpVar() {
        return st.get(st.size() - 1).size() + 3;
    }
